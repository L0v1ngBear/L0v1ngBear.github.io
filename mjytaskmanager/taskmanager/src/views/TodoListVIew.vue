<template>
  <div class="todo-list-container">
    <h1 class="todo-list-title">代办事项</h1>
    <!-- 添加搜索框 -->
    <el-input
      v-model="searchQuery"
      placeholder="搜索任务名称"
    ...
    />
    <!-- 添加加载状态 -->
    <el-table :data="filteredTasks" style="width: 100%; margin-top: 20px;" :loading="isLoading">
    ...
    />
    <!-- 添加空状态提示 -->
    <el-empty v-if="filteredTasks.length === 0 && !isLoading" description="暂无任务"></el-empty>
...
const fetchTasks = async () => {
  ...
  try {
    const response = await request.get(`/api/tasks/selectPageById/${userId}`, {
      params: {
    ...
    // 更新代办事项数量
    todoStore.setTodoCount(tasks.value.total);
  } catch (error) {
...
const filteredTasks = computed(() => {
  if (!searchQuery.value) return tasks.value.list;
  return tasks.value.list.filter(task =>
